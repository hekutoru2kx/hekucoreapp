...
// through HekucoreappDbContext/EF, so a broken DbContext or exhausted EF pool isn't also what
// breaks logging. Log calls never block on a DB round-trip: Emit() only queues; a background loop
// drains and batches the writes. If the channel fills (DB slow/down) the oldest queued event is
// dropped rather than the caller being blocked — a struggling DB must never back-pressure the app.
// Any write failure is caught and swallowed here; the console/file sinks configured alongside this
// one are the fallback of record for exactly that case (this sink must never be the only sink).
public class SystemLogPostgresSink : ILogEventSink, IDisposable
{
    ...
    private async Task WriteBatchAsync(List<LogEvent> batch, CancellationToken cancellationToken)
    {
        ...
        foreach (var evt in batch)
        {
            ...
            cmd.CommandText = """
                INSERT INTO system_logs (timestamp, level, category, message, exception, user_id, trace_id)
                VALUES (@timestamp, @level, @category, @message, @exception, @user_id, @trace_id)
                """;
