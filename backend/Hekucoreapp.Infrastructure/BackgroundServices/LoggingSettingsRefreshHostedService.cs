...
namespace Hekucoreapp.Infrastructure.BackgroundServices;

// Keeps the live CategoryLevelSwitches in sync with the admin-editable LoggingCategorySettings
// rows — a fixed PeriodicTimer tick — so a change takes effect without a restart. Runs the daily
// system_logs retention sweep off the same timer rather than a second hosted service; system_logs
// isn't part of the EF model (see the CreateSystemLogsTable migration), so the sweep is a raw
// DELETE, same as SystemLogPostgresSink's writes.
