...
// audit trail for auth events, so an admin can loosen it (more verbose) but not silence it.
public class LoggingSettingsRepository : ILoggingSettingsRepository
{
    ...
    public LoggingSettingsRepository(HekucoreappDbContext context, IStringLocalizer<Messages> localizer)
    {
        _context = context;
