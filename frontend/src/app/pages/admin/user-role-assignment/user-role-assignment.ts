...
import { Admin, RoleAssignmentInput } from '../../../services/admin';
...
// Per-user role editor: one row per currently-open assignment, each with an inline start/expiry
// window. Adapted from the sibling projects' employee-role-assignment page (there it is keyed off
// a tenant Employee; here off a plain User). Save diffs the whole set — see
// UserRoleRepository.AssignRolesAsync.
@Component({
...
export class UserRoleAssignment implements OnInit {
  ...
  ngOnInit(): void {
    ...
    forkJoin({
      ...
      assignments: this.adminService.getRoleAssignments(this.userId),
      roles: this.rolesService.getRoles()
    }).subscribe({
      next: ({ user, assignments, roles }) => {
        ...
        this.availableRoles.set(roles.map(r => r.name).filter(r => !heldNames.has(r)).sort());
  ...
  save(): void {
    ...
    this.adminService.assignRoles(this.userId, roles).subscribe({
