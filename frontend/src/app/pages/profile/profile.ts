...
export class Profile implements OnInit {
  ...
  onSubmit(): void {
    ...
    const { userName, email, preferredLanguage, preferredTheme } = this.form.value;

    this.http.put(
      `${environment.apiUrl}/user/profile`,
      { userName, email, preferredLanguage, preferredTheme }
    ).subscribe({
      next: () => {
        ...
        if (current) {
          ...
          localStorage.setItem('hekucoreapp_user', JSON.stringify(updated));
        }
